...
<link rel="preconnect" href="//chart.googleapis.com">
_HTML_;
echo "\r\n";
...
for ($i = 0, $num_domains = count($hostarray); $i < $num_domains; $i++) {
	echo "<link rel=\"preconnect\" href=\"//".$hostarray[$i]."\">\r\n";
	}
...
$desktop_psi_lookup = json_decode(file_get_contents("https://www.googleapis.com/pagespeedonline/v2/runPagespeed?url=http%3A%2F%2F".$hostarray[$i]."&strategy=desktop&key=".$key), true);
$desktop_psi_lookup_score = (int) $desktop_psi_lookup['ruleGroups']['SPEED']['score'];
//begin the mobile query
$mobile_psi_lookup = json_decode(file_get_contents("https://www.googleapis.com/pagespeedonline/v2/runPagespeed?url=http%3A%2F%2F".$hostarray[$i]."&strategy=mobile&key=".$key), true);
$mobile_psi_lookup_score = (int) $mobile_psi_lookup['ruleGroups']['SPEED']['score'];
echo "<tr>";
echo "<td>";
